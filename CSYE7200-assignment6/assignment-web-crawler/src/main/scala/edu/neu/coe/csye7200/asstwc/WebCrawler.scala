...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 16 points.
    def getURLs(ns: Node): Seq[Try[URL]] = for(n <- ns \\"a"; href = n\@"href") yield createURL(Option(u),href) // TO
    // BE
    // IMPLEMENTED
    ...
    // 9 points.
    for(url <- getURLContent(u); links <- MonadOps.asFuture(getLinks(url))) yield links
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // 15 points. Implement the rest of this, based on us2 instead of
    // us.
    // TO BE IMPLEMENTED
    //Future.sequence(MonadOps.mapFuture(for(u <- us2) yield wget(u)))
